...
def _() -> str:
    ...
    #
    # Сверяется сырой процент, а не сглаженный. EMA по замыслу зависит от
    # предыдущих замеров: на пустой базе она совпадает с сырым значением, а на
    # базе с историей — нет, и проверка сглаженного числа падала бы не потому,
    # что стенд сломан, а потому что до неё уже работали.
    if abs(body["fill_raw_percent"] - 50.0) > 0.5:
        raise RuntimeError(
            f"16 см должны дать 50%, получено {body['fill_raw_percent']}"
        )
    return (
        f"16 см -> {body['fill_raw_percent']}% сырых, "
        f"{body['fill_percent']}% после сглаживания"
    )
